...
class MatchResults:
    ...
    def get_names(self):
        with open(self._names) as f:
            lst = json.load(f, encoding="ascii")
        return [dct["name"] for dct in lst]

    def get_matches(self):
        with open(self._match) as f:
            lst = json.load(f)
        return [[int(dct["team1"]), int(dct["team2"]),
                 int(dct["balls1"]), int(dct["balls2"])] for dct in lst]
        ...
        # http://127.0.0.1:8000/operation_status
            ...
            if name_1 and name_2 and balls_1 and balls_2:
                try:
                    ...
                    new_match = dict()
                    new_match["team1"] = str(pos_1 + 1)
                    new_match["team2"] = str(pos_2 + 1)
                    new_match["balls1"] = str(balls_1)
                    new_match["balls2"] = str(balls_2)

                    with open(self._match, "r") as f:
                        lst = json.load(f)
                    with open(self._match, "w") as f:
                        lst.append(new_match)
...
if __name__ == '__main__':
    app = MatchResults("data/names.json", "data/results.json")
